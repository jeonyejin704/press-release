...
| 초안·사진을 Outlook 메일로 별도 수신 | 유형별 **초안 양식 다운로드 → 작성 → 업로드**, 홍보팀이 보완본 재업로드 |
...
- **홍보 신청**: 6개 유형별 안내문(줄글) + 기본 정보 폼, 임시저장·제출
- **보도자료 초안(파일 기반)**: 유형별 **초안 양식 다운로드** → 신청자 작성·업로드 → 홍보팀 **보완본 재업로드**
- **보도자료 에디터(선택)**: 국문·영문 최종본 in-app 작성, 미리보기, **버전 이력/변경자/변경사유**
...
**빠른 시나리오**
1. `prof.kim`으로 로그인 → 새 홍보 신청 → 연구성과 → **초안 양식 다운로드** → 작성 후 파일 업로드 → 제출
2. `pr.manager`로 로그인 → **대시보드(오늘 배포 보도자료 확인)** → 신청 상세
3. 보도자료 탭에서 초안 파일 다운로드 → 보완본 재업로드 → 배포일 안내 → 최종본 확정
4. 뉴스 모니터링 → 새로고침 → 중요 표시

> 참고: AI 자동 초안 생성은 기본 비활성화(파일 기반 흐름). 필요 시 `AI_PROVIDER`/`POST /api/ai/generate-press-release`로 옵션 활용 가능합니다.
